...
actual fun TimerScreenRoot(
    ...
    BackHandler(true) {
        if (state.value.timerStatus!= TimerStatus.Ready) {
            showExitDialog = true
        } else {
            closeTimerScreen()
        }
    ...
    Box(
        ...
        Row(
                    ...
                    BasicText(
                        ...
                        style = TextStyle(
                            shadow = Shadow(
                                ...
                                offset = Offset(0f, 4f),
                                blurRadius = if (state.timerStatus == Running) 30f else 0f
                            ),
                ...
                Box(
                    ...
                    BasicText(
                        ...
                        style = TextStyle(
                            ...
                            letterSpacing = 6.sp,
                            textAlign = TextAlign.Center,
                            color = if (state.timerStatus == Running) MaterialTheme.colorScheme.secondary else MaterialTheme.colorScheme.onSurfaceVariant.copy(alpha = 0.7f)
                        ),
            ...
            Column(
                ...
                RectangleButton(
                    modifier = Modifier
                        .weight(if (state.timerStatus.isInActiveState()) 2f else 1f),
                    isActive = state.timerStatus.isInActiveState(),
                    onButtonClicked = {
                        if (state.timerStatus.isInActiveState()) {
                    ...
                    text = if (state.timerStatus == Running) stringResource(Res.string.button_pause) else stringResource(
    ...
    Scaffold(
        ...
        Column(
            ...
            Column(
                ...
                RectangleButton(
                    modifier = Modifier
                        .weight(if (state.timerStatus.isInActiveState()) 4f else 3f),
                    isActive = state.timerStatus.isInActiveState(),
                    onButtonClicked = {
                        if (state.timerStatus.isInActiveState()) {
                            onAction(TimerActions.PauseTimer)
                    ...
                    activeTextColor = White,
                    text = when {
                        state.timerStatus.isInActiveState() -> stringResource(Res.string.button_pause)
